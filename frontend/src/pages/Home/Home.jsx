...
const Home = () => {
  ...
  useEffect(() => {
    const checkPopup = async () => {
      ...
      if (data.success) {
        // Tìm quảng cáo loại popup và đang ACTIVE
        const popupAd = data.ads.find(
          (ad) => ad.type === "popup" && ad.status === "ACTIVE",
        );
        if (popupAd) {
          // Gọi hàm mở Modal hiển thị ảnh popupAd.image ở đây
          console.log("Hiện quảng cáo:", popupAd.title);
        }
      }
    };
    checkPopup();
